...
def breach_map(dam: DamObject, f: float, level: float, dhdt: float,
               connectivity: int = 8, out_dir: str | Path = ".",
               label_source: str = "source lake", label_target: str = "target lake") -> dict:
    ...
    path = minimax_path(np.where(np.isfinite(phi), phi, np.inf), T,
                        dam.seed_lake, dam.seed_target, connectivity)
    ...
    extent = [x0 / 1000, (x0 + ncols * dx) / 1000,
              (y0 + nrows * dy) / 1000, y0 / 1000]  # km, north up

    fig, ax = plt.subplots(figsize=(8.4, 8.0), facecolor=SURFACE)
    ax.set_facecolor(SURFACE)
    ax.imshow(phi, extent=extent, origin="upper", cmap="Greys",
              vmin=np.nanpercentile(phi, 1), vmax=np.nanpercentile(phi, 99),
              interpolation="nearest")
    for mask, colour in ((tgt, TARGET), (src, SOURCE)):
        ax.imshow(np.where(mask, 1.0, np.nan), extent=extent, origin="upper",
                  cmap=matplotlib.colors.ListedColormap([colour]), alpha=0.55,
                  interpolation="nearest")
    if path.size:
    ...
    def mark(cell, text, colour, dxy=(0.25, 0.25)):
        ...
        ax.annotate(text, (mx / 1000 + dxy[0], my / 1000 + dxy[1]), color=INK,
                    fontsize=10, fontweight="medium", zorder=7,
    ...
    ax.annotate(f"breach point\nΦ = {T:.1f} m", (mx / 1000 + 0.3, my / 1000 - 0.55),
                color=INK, fontsize=10, fontweight="semibold", zorder=9,
                bbox=dict(boxstyle="round,pad=0.3", fc=SURFACE, ec=INK_2, lw=0.6, alpha=0.9))

    year = dam.base_year + crit.breach_time(
        np.array([T]), np.array([level]), np.array([dhdt]), np.array([f]))[0]
    ax.set_title(f"{dam.name}: breach pathway   —   {np.ceil(year):.0f}  (crossing {year:.2f})",
                 color=INK, fontsize=13, pad=22, loc="left")
    ...
    ax.legend(
        handles=[
            Line2D([], [], marker="s", ls="", ms=10, mfc=TARGET, mec="none",
                   label=f"{label_target} basin"),
            Line2D([], [], marker="s", ls="", ms=10, mfc=SOURCE, mec="none",
                   label=f"{label_source} basin"),
            Line2D([], [], color=PATH, lw=2.4, label="breach pathway"),
            Line2D([], [], marker="o", ls="", ms=10, mfc="none", mec=INK, mew=2,
                   label="breach point (saddle)"),
        ],
        loc="lower left", frameon=True, facecolor=SURFACE, edgecolor="#d8d7d2",
    ...
    rows = [("seed_source", *dam.seed_lake), ("seed_target", *dam.seed_target),
            ("breach_point", *crest)]
    pts.write_text(
        "name,row,col,x,y,phi_m\n"
        + "".join(f"{n},{r},{c},{dam.xy((r, c))[0]:.3f},{dam.xy((r, c))[1]:.3f},"
                 f"{phi[r, c]:.4f}\n" for n, r, c in rows)
    )
    line = out_dir / f"breach_pathway_{dam.name}.csv"
    line.write_text(
        "order,row,col,x,y,phi_m\n"
        + "".join(f"{i},{r},{c},{dam.xy((r, c))[0]:.3f},{dam.xy((r, c))[1]:.3f},"
                 f"{phi[r, c]:.4f}\n" for i, (r, c) in enumerate(map(tuple, path)))
    )
    return {"map": png, "points": pts, "pathway": line, "phi_crit": T, "breach_year": year,
            "crest": crest}
